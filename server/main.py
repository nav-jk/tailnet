...
async def get_client(db: aiosqlite.Connection, username: str) -> dict | None:
    ...
    async with db.execute(
        "SELECT username, ip, port, last_seen FROM clients WHERE username = ?",
        (username,)
    ) as cur:
    ...
    username, ip, port, last_seen = row
...
async def register(client: Client):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("""
            INSERT INTO clients (username, ip, port, last_seen)
            VALUES (?, ?, ?, ?)
            ON CONFLICT(username) DO UPDATE SET
                ip        = excluded.ip,
                port      = excluded.port,
                last_seen = excluded.last_seen
        """, (client.username, str(client.ip), client.port, now()))
        await db.commit()
